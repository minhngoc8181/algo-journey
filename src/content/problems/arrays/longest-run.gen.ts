...
export default defineTests('longest-run', (t, rng) => {
  ...
  // ── Hidden Tests ──
  t.hidden('several-long-runs', { args: [[1, 1, 1, 2, 2, 3, 1, 1, 1, 2, 2, 3, 1, 1, 1]], expected: 3 });
  ...
  // ── Generated Tests ──
  function computeRun(arr: number[]): number {
    let best = 1, run = 1;
    for (let k = 1; k < arr.length; k++) {
      if (arr[k] === arr[k-1]) { run++; if (run > best) best = run; }
      else run = 1;
    }
    return best;
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let k = 0; k < len; k++) {
      if (rng.bool(0.3)) currentVal = rng.int(0, 5);
      testArr.push(currentVal);
    }
    t.hidden(`gen-${i}`, { args: [testArr], expected: computeRun(testArr) });
  }
});
