...
export default defineTests('min-stack', (t, rng) => {
  ...
  // Generated tests – use seeded rng (was Math.random() before)
  for (let i = 0; i < 15; i++) {
    const ops: any[][] = [['new']];
    const expectedOut: number[] = [];
    const stack: number[] = [];
    const minTrack: number[] = [];

    const numOps = rng.int(10, 100);
    for (let k = 0; k < numOps; k++) {
      const p = rng.int(0, 99) / 100;
      if (p < 0.5 || stack.length === 0) {
        const val = rng.int(-1000, 1000);
        ops.push(['push', val]);
        stack.push(val);
        if (minTrack.length === 0 || val <= minTrack[minTrack.length - 1]!) {
          minTrack.push(val);
        } else {
          minTrack.push(minTrack[minTrack.length - 1]!);
        }
      } else if (p < 0.7) {
        ops.push(['pop']);
        stack.pop();
        ...
        ops.push(['getMin']);
        expectedOut.push(minTrack[minTrack.length - 1]!);
      }
    }
    const flat = expectedOut.length === 1 ? expectedOut[0] : expectedOut;
    t.hidden(`gen-${i}`, { operations: ops, expected: flat });
  }
});
